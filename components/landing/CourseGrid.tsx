...
export function CourseGrid() {
  ...
  const [tutors, setTutors] = useState<FacultyTutor[]>(DEFAULT_FACULTY_TUTORS);
        ...
        if (res.ok) {
          ...
          if (data.courses && data.courses.length > 0) {
            const formatted = data.courses.map((c: any) => ({
              ...
              rating: 4.98,
              reviewCount: 1240,
              studentsEnrolled: c.enrollments?.length
                ? c.enrollments.length * 1420 + 200
                : 4800,
              durationHours: 58,
              lessonsCount:
                c.modules?.reduce(
